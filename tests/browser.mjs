...
try {
  ...
  await page.locator('.bar-group').filter({ hasText: '01.09' }).click(); assert.ok(await page.getByRole('dialog').isVisible()); await page.getByRole('button', { name: 'Закрыть', exact: true }).last().click()
  ...
  for (const source of ['Metrix', 'Webkassa']) {
    ...
    assert.equal(await page.locator('tbody tr').count(), 7)
    await page.locator('.bar-group').first().click()
    const detailDialog = page.getByRole('dialog')
    assert.ok(await detailDialog.isVisible())
    assert.equal(await detailDialog.locator('h2').innerText(), await page.locator('tbody tr').first().locator('td b').innerText())
    assert.equal(await detailDialog.locator('.detail-list dd').first().innerText(), `${await page.locator('tbody tr').first().locator('td').nth(1).innerText()} ₸`)
    assert.equal(await detailDialog.getByText('Разница', { exact: true }).count(), 0)
    assert.equal(await detailDialog.getByText('Чеков учтено', { exact: true }).count(), source === 'Webkassa' ? 1 : 0)
    await page.keyboard.press('Escape')
    assert.equal(await page.getByRole('dialog').count(), 0)
    assert.ok(await page.locator('.bar-group').first().evaluate(el => el === document.activeElement))
